    ...
    <div class="container-xl">
        <div class="card">
            ...
            <div class="card-body">
              ...
              <div class="card-footer text-start d-flex gap-2">
                @if ($purchase->purchase_status === 0)
                    <form action="{{ route('purchases.received', $purchase) }}" method="POST">
                        @csrf
                        @method('put')
                ...
                @endif
                 @if ($purchase->payment_status === 1)
                    <form action="{{ route('purchases.paid', $purchase) }}" method="POST">
                        @csrf
                        @method('put')
                        <input type="hidden" name="id" value="{{ $purchase->id }}">
...
<script>
    document.getElementById('paidPurchaseBtn').addEventListener('click', function(e) {
        e.preventDefault();

        Swal.fire({
            title: 'Are you sure?',
            text: "Do you want to mark this purchase as paid?",
            icon: 'warning',
...
<script>
    document.getElementById('receivePurchaseBtn').addEventListener('click', function(e) {
        e.preventDefault();

        Swal.fire({
            title: 'Are you sure?',
            text: "Do you want to mark this purchase as received?",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
